...
Autonomous::Autonomous(JrimmyGyro *Gyro, frc::Joystick *ButtonBox, SwerveDrive *SwerveDrive, CFS *CFS, LimeyLight *Lime) : a_Gyro(Gyro),
...
void Autonomous::AutonomousPeriodic0()
{
    ...
    case kDriveAway0:
        if (!IHaveAProposal(0.3, 30.0, TO_TRENCH_FROM_LINE))
        { // DriveDist(ARBITRARY_DIST_BACKWARDS, 180)){
...
bool Autonomous::DriveDist(double dist, double angle)
...
bool Autonomous::IHaveAProposal(float speed, float dir, float dist)
{ // true is done, false is not done
...
bool Autonomous::GoToMcDonalds(float speed, float dir, float dist)
{ // true is done, false is not done
