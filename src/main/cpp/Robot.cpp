...
a_FLModule(FL_DRIVE_ID, FL_STEER_ID, FL_SWERVE_ABS_ENC_PORT, FL_SWERVE_ABS_ENC_OFFSET),
a_FRModule(FR_DRIVE_ID, FR_STEER_ID, FR_SWERVE_ABS_ENC_PORT, FR_SWERVE_ABS_ENC_OFFSET), // (when we get analog encoders, replace 1-4 with actual IDs)
a_BLModule(BL_DRIVE_ID, BL_STEER_ID, BL_SWERVE_ABS_ENC_PORT, BL_SWERVE_ABS_ENC_OFFSET),
a_BRModule(BR_DRIVE_ID, BR_STEER_ID, BR_SWERVE_ABS_ENC_PORT, BR_SWERVE_ABS_ENC_OFFSET),
...
void Robot::TeleopPeriodic() // main loop
{
    ...
    if (a_XboxController.GetRawButton(OperatorButton::Y)) {
        ...
        // might not be necesary, but we need to test with no failures so add just in case
        a_Collector.setMotorSpeed(0);
    }
